...
def render_viewpoint(model,
    ...
    with torch.no_grad():
        render_result_chunks = [
            ...
            for ro, rd, vd in zip(rays_o.flatten(0,-2).split(n_batch, 0),
                                  rays_d.flatten(0,-2).split(n_batch, 0),
                                  viewdirs.flatten(0,-2).split(n_batch, 0))
        ]
        render_result = {
    ...
    return rgb, rays_o, rays_d, viewdirs