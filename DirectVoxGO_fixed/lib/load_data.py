...
from .load_mareva import load_mareva_data
...
def load_data(args):
    ...
    if args.dataset_type == 'llff':
        ...
        images, poses, render_poses, hwf, i_split = load_dv_data(scene=args.scene,
    ...
    elif args.dataset_type == 'mareva':
        images, poses, render_poses, hwf, K, i_split = load_mareva_data(args.datadir)
        print('Loaded mareva', images.shape, render_poses.shape, hwf, args.datadir)
        i_train, i_val, i_test = i_split
    ...
    print(f'poses.shape: {poses.shape}')  # n_cameras * 4 * 4
    print(f'Height, Width (image 0): {HW[0]}')
    print(f'Focal: {focal}')
    print(f'irregular_shape: {irregular_shape}')
    print(f'near: {near}')
    print(f'far: {far}')
    print(f'depths: {depths}')  # expected None here

    render_poses = render_poses[..., :4]

    data_dict = dict(hwf=hwf,
                     HW=HW, Ks=Ks,
                     near=near,
                     far=far,
                     i_train=i_train,
